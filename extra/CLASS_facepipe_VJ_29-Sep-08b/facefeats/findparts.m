% FINDPARTS     Find parts (facial features)
%
% P = findparts(model,I,det) finds the facial features for the given image
% and face detection, returning their 2-D coordinates in columns of P
%
% I should be a uint8 grayscale (h x w) or RGB (h x w x 3) image
% det specifies the face output of a face detector:
%   det(1:2) is the (x,y) coordinates of the center of the face detection
%   det(3) is the scale as half-width of the face detection square

function Pbest = findparts(model,I,det)

[J,T]=extface(model,I,det);
...
Pbest=[T * [Pbest ; ones(1,size(Pbest,2))]];
